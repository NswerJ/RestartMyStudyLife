...
namespace RL.Core.Editors
{
    ...
    public class RMSL_BaseGraphView : GraphView
    {
        public T AddNode<T>() where T : RMSL_BaseNode, new()
        {

            return AddNode<T>("New Node");

        }
        public T AddNode<T>(string titleText) where T : RMSL_BaseNode, new()
        {

            return AddNode<T>(titleText, new Vector2(300, 300));

        }
        public T AddNode<T>(string titleText, Vector2 size) where T : RMSL_BaseNode, new()
        {

            return AddNode<T>(titleText, size, Vector2.zero);

        }
        public T AddNode<T>(string titleText, Vector2 size, Vector2 position) where T : RMSL_BaseNode, new()
        {

            return AddNode<T>(titleText, size, position, true, true);

        }
        public T AddNode<T>(string titleText, Vector2 size, Vector2 position, bool movable, bool deletable) where T : RMSL_BaseNode, new()
        {
